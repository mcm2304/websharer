...
async function loadPosts(){
    ...
    let postsHtml = postsJson.map(postInfo => {
        return `
            <div class="post">
                <p class="post-text" style="margin: 0.5rem;"><strong>Username:</strong> ${postInfo.username}</p>
                <p class="post-text" style="margin: 0 0.5rem;"><strong>Description:</strong> ${postInfo.description}</p>
...
async function postUrl(){
    ...
    try{
        ...
        document.getElementById("postStatus").innerText = "Error: Invalid URL"
        throw(error)
    }
